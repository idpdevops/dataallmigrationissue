...
class PipelineStack(Stack):
    def __init__(
       ...
       self.pipeline_bucket = s3.Bucket(
        ...
        # Bespoke Integration changes start
        # Bespoke Integration changes start
        # Bespoke Integration changes start
        tags = [
            {
                'Application': f'{resource_prefix}-{git_branch}'
            }
        ]

        if self.node.try_get_context('tooling_tags'):
            custom_tooling_tags = self.node.try_get_context('tooling_tags')
            tags = [*tags, *custom_tooling_tags]
            
        for tag_key_value_pair in tags:
            for key, value in tag_key_value_pair.items():
                Tags.of(self).add(key, value)
        # Tags.of(self).add('Application', f'{resource_prefix}-{git_branch}')
        # Bespoke Integration changes end
        # Bespoke Integration changes end
        # Bespoke Integration changes end
